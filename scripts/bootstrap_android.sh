...
if [[ ! -f "android/app/build.gradle.kts" && ! -f "android/app/build.gradle" ]]; then
  ...
  # flutter create also generates the template widget_test.dart, which references MyApp.
  # KickLuck has its own tests/app entrypoint, so remove only that generated template.
